...
string S3fsCurl::CalcSignatureV2(const string& method, const string& strMD5, const string& content_type, const string& date, const string& resource, const string& content_length)
{
  string Signature;
  string StringToSign;
  ...
  StringToSign += s3fs_md5sum(S3fsCurl::UpYunPassword);

  return s3fs_md5sum(StringToSign);
}
...
int S3fsCurl::HeadRequest(const char* tpath, headers_t& meta)
{
  ...
  for(headers_t::iterator iter = responseHeaders.begin(); iter != responseHeaders.end(); ++iter){
    ...
    if(key == "content-type"){
      ...
      meta[iter->first] = value;
    }else if(key.substr(0, 7) == "x-upyun"){
...
int S3fsCurl::PutRequest(const char* tpath, headers_t& meta, int fd)
{
  ...
  if(!S3fsCurl::IsPublicBucket()){
    ...
    string Signature = CalcSignatureV2("PUT", strMD5, ContentType, date, resource, ss.str());
    requestHeaders   = curl_slist_sort_insert(requestHeaders, "Authorization", string("UpYun " + UpYunUsername + ":" + Signature).c_str());
  }
...
int S3fsCurl::ListBucketRequest(const char* tpath, const char* query)
{
  ...
  MakeUrlResource(tpath, resource, turl);    // NOTICE: path is "".
  if(query){
    requestHeaders = curl_slist_sort_insert(requestHeaders, "X-List-Iter", query);
  }
