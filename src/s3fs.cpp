...
static int check_for_upyun_format(void);
...
static int chk_dir_object_type(const char* path, string& newpath, string& nowpath, string& nowcache, headers_t* pmeta, int* pDirType)
{
  ...
  if(0 == (result = get_object_attribute(newpath.c_str(), NULL, pmeta, false, &isforce))){
    ...
    if(0 == (result = get_object_attribute(nowpath.c_str(), NULL, pmeta, false, &isforce))){
      ...
      if(-ENOTEMPTY == directory_empty(newpath.c_str())){
        // "no dir object" type.
        (*pType) = DIRTYPE_NOOBJ;
        nowpath  = "";            // now path.
    ...
    if('/' != strpath[strpath.length() - 1]){
      ...
      if(is_need_check_obj_detail(*pheader)){
        if(-ENOTEMPTY == directory_empty(strpath.c_str())){
          ...
          headers_t::const_iterator iter = (*pheader).find("x-upyun-file-type");
          if (iter != (*pheader).end()) {
            if (iter->second == "folder") {
              strpath += "/";
              forcedir =true;
              if (pisforce)
                (*pisforce) = true;
  ...
  for(mn_cur = mn_head; mn_cur; mn_cur = mn_cur->next){
    if(!mn_cur->is_dir){
      if(nocopyapi || norenameapi){
	    ...
	    S3FS_PRN_EXIT("upyunfs don't support copy api, so exit.");
      }
      if(0 != result){
...
static int list_bucket(const char* path, S3ObjList& head, const char* delimiter, bool check_content_only)
{
  ...
  while(truncated){
    // request
    if(0 != (result = s3fscurl.ListBucketRequest(path, query))){
      S3FS_PRN_ERR("ListBucketRequest returns with error.");
      return result;
    }
    for (headers_t::iterator iter = (s3fscurl.GetResponseHeaders())->begin(); iter != (s3fscurl.GetResponseHeaders())->end(); ++iter) {
      if (iter->first == "x-upyun-list-iter") {
        if (iter->second == "g2gCZAAEbmV4dGQAA2VvZg")
          truncated = false;
        else
          query = iter->second.c_str();
      }
    }
    ...
    for(std::vector<std::string>::iterator iter = vLine.begin(); iter != vLine.end(); ++iter) {
       vItem.clear();
       split(*iter, "\t", vItem);
       if (vItem.size() != 4)
           continue;
       ...
       if (vItem[1] == "F") {
           head.insert(vItem[0].c_str(), NULL, true);
       }
       else if (vItem[1] == "N") {
           head.insert(vItem[0].c_str(), NULL, false);
       }
    }
    truncated = false;
...
static int check_for_upyun_format(void)
{
  ...
  string str1 ("UpYunUsername=");
  string str2 ("UpYunPassword=");
  size_t found;
  string AccessKeyId;
...
static int check_passwd_file_perms(void)
{
  ...
  // /etc/passwd-upyunfs can have group permissions 
  if(passwd_file != "/etc/passwd-upyunfs"){
    ...
    // "/etc/passwd-upyunfs" does not allow group write.
...
static int read_passwd_file(void)
{
  ...
  upyun_format = check_for_upyun_format();
  if(1 == upyun_format){
     return EXIT_SUCCESS;
  }else if(-1 == upyun_format){
     return EXIT_FAILURE;
  }
...
// 4 - from the users ~/.passwd-upyunfs
// 5 - from /etc/passwd-upyunfs
//
static int get_access_keys(void)
{
  // should be redundant
  ...
  char* UpYunUsername     = getenv("UPYUNUSERNAME");
  char* UpYunPassword = getenv("UPYUNPASSWORD");
  if(UpYunUsername != NULL || UpYunPassword != NULL){
    if( (UpYunUsername == NULL && UpYunPassword != NULL) ||
        (UpYunUsername != NULL && UpYunPassword == NULL) ){
      S3FS_PRN_EXIT("if environment variable UpYunUsername is set then UpYunPassword must be set too.");
      return EXIT_FAILURE;
    }
    if(!S3fsCurl::SetAccessKey(UpYunUsername, UpYunPassword)){
      S3FS_PRN_EXIT("if one access key is specified, both keys need to be specified.");
      return EXIT_FAILURE;
    }
    return EXIT_SUCCESS;
  ...
  // 3a - from the UPYUN_CREDENTIAL_FILE environment variable
  char * UPYUN_CREDENTIAL_FILE;
  UPYUN_CREDENTIAL_FILE = getenv("UPYUN_CREDENTIAL_FILE");
  if(UPYUN_CREDENTIAL_FILE != NULL){
    passwd_file.assign(UPYUN_CREDENTIAL_FILE);
    if(passwd_file.size() > 0){
      ifstream PF(passwd_file.c_str());
      if(PF.good()){
         PF.close();
        ...
        S3FS_PRN_EXIT("UPYUN_CREDENTIAL_FILE: \"%s\" is not readable.", passwd_file.c_str());
        return EXIT_FAILURE;
      }
    }
  }
  ...
  if(HOME != NULL){
     ...
     passwd_file.append("/.passwd-upyunfs");
     ifstream PF(passwd_file.c_str());
     if(PF.good()){
       PF.close();
  ...
  passwd_file.assign("/etc/passwd-upyunfs"); 
  ifstream PF(passwd_file.c_str());
  if(PF.good()){
    PF.close();
    return read_passwd_file();
...
static int my_fuse_opt_proc(void* data, const char* arg, int key, struct fuse_args* outargs)
{
  if(key == FUSE_OPT_KEY_NONOPT){
    ...
    if(0 == STR2NCMP(arg, "iam_role")){
      S3FS_PRN_EXIT("option iam_role is no needed by upyun.");
      return 0;
    }
    ...
    if(0 == strcmp(arg, "noxmlns")){
      S3FS_PRN_EXIT("option noxmlns is no needed by upyun.");
      return 0;
    }
    if(0 == strcmp(arg, "nocopyapi")){
      S3FS_PRN_ERR("upyunfs don't have copy api, so always true.");
      return 0;
    }
    if(0 == strcmp(arg, "norenameapi")){
      S3FS_PRN_ERR("upyunfs don't have rename api, so always true.");
      return 0;
    }
